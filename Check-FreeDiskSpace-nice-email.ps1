$servers = Get-ADComputer -Filter * -SearchBase "OU=Servers,DC=DOMAIN,DC=BIZ" | Select-Object -ExpandProperty name
#$path = 'C:\PSscripts\rdrivespace.txt'
#$pathcsv = 'C:\PSscripts\export.csv'
$log = 'C:\PSscripts\rdrivespace.log'
#$recepients = @('cduran@tcfbank.com')
$recepients = @('cduran@company.com','exchangegroup@company.com')
$PSEmailServer = 'smtp.company.biz'
...
function Drivespace{ param ([string] $server)
    ...
    Invoke-sqlcmd -Query $InsertResults -ServerInstance MN-SQLSERVER -Database DiskMonitor 
...
$Script:html += "<br /> If you have any questions please let me know.<br /> Thank you,<br /> <br /> Exchange Server Team <br />"
$Script:html

#$body = Get-Content -Path $path -Raw
if(Test-Path $log){
send-MailMessage -To $recepients -From relayserver@company.com -Subject "Servers Low on Free Space Reno" -body  $Script:html -Attachments $log -BodyAsHtml
Remove-Item $log
}
else{
Send-MailMessage -To $recepients -From relayserver@company.com -Subject "Servers Low on Free Space Reno" -body $Script:html -BodyAsHtml
#Remove-Item $path
}
